...
Vamos criar uma variante do mesmo gráfico mostrando o total acumulado de respostas no tempo.
...
# Criamos a série de respostas acumuladas ao longo do tempo
df_sorted = df.sort_values("Timestamp")
df_sorted["Total Acumulado"] = df_sorted["Count"].cumsum()

# Criamos o gráfico de linha para exibir o total acumulado de respostas
fig = go.Figure()

fig.add_trace(go.Scatter(
    x=df_sorted["Timestamp"],
    y=df_sorted["Total Acumulado"],
    mode="lines",
...
# Ajustamos a aparência da linha
fig.update_traces(marker_line_color="rgba(0,80,180,0.25)", marker_line_width=2)

# Customizamos o eixo X
fig.update_xaxes(
    tickformat="%d/%m %H:%M", title_text="Data e Hora",
    dtick=6 * 3600 * 1000,  # 6 horas em milissegundos
    tickangle=90
)

# Customizamos o eixo Y
fig.update_yaxes(title_text="Total Acumulado de Respostas")
...
for inicioNoite, fimNoite in noites:
    fig.add_shape(type="rect",
                  x0=inicioNoite, x1=fimNoite, y0=0, y1=df_sorted["Total Acumulado"].max(),
                  fillcolor="darkgray", opacity=0.5, layer="below", line_width=0)

# Criamos anotações para o fim de semana
inicioFimDeSemana = pd.Timestamp("2025-01-31 18:00:00")  
fimFimDeSemana = pd.Timestamp("2025-02-03 06:00:00") 
...
fig.add_shape(type="rect",
              x0=inicioFimDeSemana, x1=fimFimDeSemana, 
              y0=0, y1=df_sorted["Total Acumulado"].max(),
              fillcolor="red", opacity=0.1, layer="below", line_width=0)
...
fig.add_trace(go.Scatter(
    x=lembretes,
    y=[df_sorted["Total Acumulado"].max() * 1.05] * len(lembretes),  
    mode="markers",
    marker=dict(symbol="triangle-down", size=10, color="red"),
    name="Lembretes",
    hovertemplate="%{x}<extra></extra>"
